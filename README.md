...
1、收藏的电子书越来越多，真正用来阅读的时间有限。

2、打开一本厚书时，不容易判断哪些内容与自己有关，也不知道从哪里开始。

3、阅读经常被打断，希望保留读到哪里、摘了什么、当时有什么想法。

一页读书会把你提供的书整理成一个能点开看的网页：有梗概、全书脑图、少量有出处的短句，也有回到原书的阅读路线。

> 欢迎来到一页读书。这是留给你的一点时间，不必急着读完。

**第一次用，按下面的“安装 → 上传 → 生成 → 打开”走就可以。** 现在是 0.3 试用版，生成需要支持 Skill、文件读取和程序运行的 AI 助手；文字导览可以单独打开；原书阅读与划线使用完整交付目录里的本地启动工具。

[开始安装](INSTALL.md) · [看截图](docs/screenshots.md) · [详细使用方法](docs/usage.md) · [测试结果](TESTING.md)

## 为什么做一页读书

这三张产品思路图，说明它想解决的阅读困扰、一次阅读怎么接起来，以及各个功能如何配合。

### 01 · 没时间读，和读不进去

![产品思路 01：时间不够时先了解主线，读不进去时从兴趣开始，被打断后接着上次读](docs/images/product-idea-01-reading-needs.png)

### 02 · 把一次阅读，顺着接起来

![产品思路 02：选一本书、先看全貌、挑一个问题、回到原文、下次继续](docs/images/product-idea-02-reading-flow.png)

### 03 · 功能可以多，主线要简单

![产品思路 03：梗概、脑图、短句和阅读路线帮助理解；书架、进度、摘录与笔记帮助继续阅读](docs/images/product-idea-03-feature-map.png)

## 这次新增了什么

现在 PDF、EPUB、MOBI、TXT 和 Markdown 都可以在交付网页里打开原书并划线、标重点、写笔记。PDF 保留分页；其他格式是文字阅读版，按章节或片段显示。返回导览后集中回看摘录，默认显示最近 4 条，支持展开、收起，并按最近修改时间排序。马蒂斯主题的回原文按钮是淡紫色，点击和展开保留柔和过渡。

[原书标注怎么用](docs/pdf-annotations.md) · [0.3 更新与检查](docs/release-0.3.md) · [小红书 REDSkill 候选版](https://github.com/belalee-ai/bela-One-page-book/tree/main/distributions/redskill)
...
| MOBI | 先尝试直接提取；读不了时用 Calibre 转换，或换 EPUB |

图片里的字不会因为文件是 PDF/EPUB 就自动变成文字。助手发现读不到的页面时，应说明缺口。加密和部分新版 MOBI 仍可能需要其他版本或转换工具。
...
- 原书阅读还会交付 `reader/`、`ebook_import.py` 和 `start_reader.py`：划线阅读时保留它们，让助手启动本机地址。

优先用桌面 Chrome 或 Edge 打开 HTML；Safari 和其他浏览器的显示与交互仍待验收。看成品不需要安装 Skill，也不需要再运行生成模型。

### 4. 按兴趣读，不用从头读到底

点“开始读”，可以展开梗概、查看脑图，或直接找阅读路线。点“查看对应原文”，会显示页序或章节位置和一小段上下文。需要在网页读原书时，启动本地阅读地址，再点“添加这本原书”选择生成导览时用的同一份文件；添加后便可阅读和标注。网页版文字阅读器不保留 EPUB/MOBI 的原版式与插图。
...
下面是我们先前本地书架原型的实际截图，帮助理解“选书”和“展开脑图”的阅读方式。**它们不是当前便携 Skill 模板的截图。** 图中的批量书架上传、人物插画与作者本地生成服务，并未随分享包提供；0.3 已加入独立多格式原书阅读和标注；制作新导览仍须先把书交给助手。
...
0.3 已在 macOS 内置 Chromium 打开原创 TXT 阅读版，自动测试覆盖多格式提取及服务边界；PDF 标注与恢复沿用 0.2 实测。真实厚书完整流程和其他浏览器仍待补充。[测试状态](TESTING.md) 里会保留具体范围。
