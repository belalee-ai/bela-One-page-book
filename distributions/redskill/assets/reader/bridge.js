// Optional local original-book reader. Files and notes stay in this browser.
(()=>{
 ...
 let dispose=()=>{},mountVersion=0,activeFile=null,opening=false;
 const positionKey=KEY+'-original-positions';
 let positions={};try{const p=JSON.parse(localStorage.getItem(positionKey)||'{}');if(p&&typeof p==='object'&&!Array.isArray(p))for(const [id,n]of Object.entries(p))if(/^(pdf|text):[a-f0-9]{64}$/.test(id)&&Number.isSafeInteger(n)&&n>0)positions[id]=n}catch{}
 // Keep positions saved by the 0.2 PDF reader.
 try{const p=JSON.parse(localStorage.getItem(KEY+'-pdf-pages')||'{}');if(p&&typeof p==='object')for(const [id,n]of Object.entries(p))if(/^pdf:[a-f0-9]{64}$/.test(id)&&Number.isSafeInteger(n)&&n>0&&!positions[id])positions[id]=n}catch{}
 const belongs=(file,book)=>file.hash?.slice(0,24)===book.id&&(!book.source_sha256||book.source_sha256===file.hash);
 readingPdf.configure({page:(id,p)=>{positions[id]=p;try{localStorage.setItem(positionKey,JSON.stringify(positions))}catch{}},finish:()=>{if(activeFile){record().originalDone=!record().originalDone;save();readingPdf.setFinished(record().originalDone)}}});
 async function openFile(file,book,page,annotationId){if(opening||current.id!==book.id)return;opening=true;try{activeFile=file;await readingPdf.open({id:file.id,title:book.guide.title,pdfPage:page||positions[file.id]||1,annotationId,originalFinishedAt:record().originalDone})}finally{opening=false}}
 ...
 async function mount(){
  ...
  if(!document.getElementById('synopsis')||!['pdf','epub','mobi','txt','md','markdown'].includes(book.source_format))return;
  const isPdf=book.source_format==='pdf';
  const tools=el('div'),open=el('button',isPdf?'打开 PDF 原书':'打开原书'),add=el('button',isPdf?'添加这本 PDF':'添加这本原书'),hint=el('p'),input=el('input');tools.id='portable-pdf-tools';tools.className='actions';hint.className='annotation-muted';hint.style.flexBasis='100%';hint.setAttribute('role','status');input.type='file';input.accept=isPdf?'.pdf,application/pdf':'.epub,.mobi,.txt,.md,.markdown,application/epub+zip,text/plain,text/markdown';input.hidden=true;tools.append(open,add,hint,input);document.querySelector('#main .actions').after(tools);
  if(location.protocol==='file:'||!window.isSecureContext||!window.indexedDB||!crypto.subtle||typeof HTMLDialogElement==='undefined'){open.disabled=add.disabled=true;hint.textContent='原书划线需要本地阅读服务和现代浏览器。请运行同目录的 start_reader.py，再打开显示的地址；现在仍可阅读导览和出处片段。';return}
  ...
  open.disabled=!file;hint.textContent=file?'选中文字即可划线、标重点、写笔记；自动保存到此浏览器。':'添加生成这份导览时使用的同一份原书文件，即可划线和记笔记。';showExcerpts();
  document.querySelectorAll('#main [data-source]').forEach(button=>button.addEventListener('click',()=>{
   ...
   const jump=el('button',isPdf?'在 PDF 中打开此页':'在原书中打开此节');jump.id='source-pdf-link';jump.onclick=()=>{document.getElementById('source').close();openFile(file,book,source.unit)};document.getElementById('source').append(jump);
  }));
  open.onclick=()=>openFile(file,book);add.onclick=()=>input.click();
  input.onchange=async()=>{const selected=input.files[0];input.value='';if(!selected)return;add.disabled=true;try{if(selected.size>(isPdf?100:60)*1024*1024)throw Error(isPdf?'PDF 超过 100 MB，请分卷':'电子书超过 60 MB，请分卷');if(selected.name.split('.').pop().toLowerCase()!==book.source_format)throw Error('请选择生成这份导览时使用的同一格式文件');const bytes=await selected.arrayBuffer(),hash=[...new Uint8Array(await crypto.subtle.digest('SHA-256',bytes))].map(x=>x.toString(16).padStart(2,'0')).join('');if(!belongs({hash},book))throw Error('这不是生成当前导览时的同一份原书文件，请切换到对应导览');const result=await readingPdf.add(selected);if(v!==mountVersion)return;file=result.meta;open.disabled=false;hint.textContent='原书已保存到此浏览器，可以开始标注。';showExcerpts();await openFile(file,book)}catch(err){if(v===mountVersion)hint.textContent=err.message||'原书添加失败，请检查文件和浏览器'}finally{add.disabled=false}};
 }
