...
class ServerTests(unittest.TestCase):
 def setUp(self):
  ...
  self.server=ThreadingHTTPServer(('127.0.0.1',0),partial(ReaderHandler,directory=str(root)));self.server.token='test-token';self.thread=threading.Thread(target=self.server.serve_forever,daemon=True);self.thread.start();self.url=f'http://127.0.0.1:{self.server.server_port}'
 ...
 def test_text_import_requires_local_token(self):
  body='一本可阅读的短文。'.encode()
  def post(headers):
   req=urllib.request.Request(self.url+'/api/import-book?format=txt',data=body,headers={'Content-Type':'application/octet-stream',**headers})
   try:
    with urllib.request.urlopen(req) as r:return r.status,r.read()
   except urllib.error.HTTPError as e:return e.code,e.read()
  self.assertEqual(post({})[0],403)
  self.assertEqual(post({'X-Book-Token':'test-token','Origin':'https://elsewhere.example'})[0],403)
  status,payload=post({'X-Book-Token':'test-token','Origin':self.url})
  self.assertEqual(status,200);self.assertEqual(__import__('json').loads(payload)['units'][0]['text'],'一本可阅读的短文。')
